...
for shot in available_shots.iterkeys():
    ...
    print "## Training..."
    start = time.time()
    os.system(".././vw --memory_tree {} --learn_at_leaf {} --leaf_example_multiplier {} -l {} -b {} -q {} -c --passes {} --loss_function {} --holdout_off {} -f {}".format(
                tree_node, learn_at_leaf, leaf_example_multiplier, lr, bits, 'ab', passes, loss, train_data, saved_model))
